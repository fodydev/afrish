...
fn main() {
    ...
    let countries = afrish::make_listbox(&root, &COUNTRY_NAMES);
    let send_label = afrish::make_label(&root);
    send_label.text("Send to country's leader:");
    let gift_1 = afrish::make_radio_button(&root, "gift", "card");
    gift_1.text(&gift_name("card")); 
    let gift_2 = afrish::make_radio_button(&root, "gift", "flowers");
    gift_2.text(&gift_name("flowers")); 
    let gift_3 = afrish::make_radio_button(&root, "gift", "nastygram");
    gift_3.text(&gift_name("nastygram"));
    let sent_label = afrish::make_label(&root);
    sent_label.anchor(afrish::Anchor::Centre);
    let status_label = afrish::make_label(&root);
    status_label.anchor(afrish::Anchor::W);
    let send = afrish::make_button(&root);
    send.text("Send Gift");
    ...
    content.grid().column(0).row(0)
        .sticky(afrish::Sticky::NESW).layout();
    ...
    gift_1.grid().row(1).column(1).sticky(afrish::Sticky::W).padx(20).layout();
    gift_2.grid().row(2).column(1).sticky(afrish::Sticky::W).padx(20).layout();
    gift_3.grid().row(3).column(1).sticky(afrish::Sticky::W).padx(20).layout();
    send.grid().row(4).column(2).padx(10).sticky(afrish::Sticky::E).layout();
    sent_label.grid().row(5).column(1).column_span(2)
        .padx(5).pady(5).sticky(afrish::Sticky::N).layout();
